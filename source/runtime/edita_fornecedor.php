...
class TEditaFornecedor extends TJanela
{
...
function __construct ($Parent, $operacao = 'i', $CodFor = null)
{
    ...
    parent::__construct ($operacao == 'i' ? 'Fornecedor - Incluir' : 'Fornecedor - Alterar',
			 null, null, 'fornecedores.png');
...
function grava_dados ()
{
    ...
    if ($this->operacao == 'i') $sql = 'call SP_Fornecedor_Inc';
    else $sql = 'call SP_Fornecedor_Alt';
    ...
    $data = $sql . '(' .
	    ...
	    String ($this->pessoa->CodTipoPessoa) . ',' .
            ...
            String ($url) . ',' .
	    String ($anotacoes) . ',' .
	    String (CommaToPoint ($limite_compra)) . ',' .
	    String ($ativo) . ',' .
	    $GLOBALS ['CodUsuario'] .
	    ');';
...
function pega_dados ()
{
    ...
    if (!$db->multi_query ('SELECT * FROM Vw_Fornecedores WHERE Id = ' . $this->CodFor)) return;
