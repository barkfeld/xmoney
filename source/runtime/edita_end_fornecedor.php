...
class TEditaEndFornecedor extends TJanela
{
 ...
 *  a => alterar
 */
function __construct ($Parent, $operacao = 'i', $CodFor, $CodId = null)
{
    ...
    // referencia
    $vbox->pack_start ($hbox = new GtkHBox);
    $hbox->pack_start (new GtkLabel ('Referencia: '), false);
...
function grava_dados ()
{
    ...
    if ($this->operacao == 'i') $sql = 'call SP_End_Fornecedor_Inc';
    else $sql = 'call SP_End_Fornecedor_Alt';
...
function pega_dados ()
{
    ...
    if (!$db->multi_query ('SELECT * FROM Vw_End_Fornecedores WHERE Id = ' . $this->CodId)) return;
