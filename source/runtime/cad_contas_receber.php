...
require_once 'baixa_contas_receber.php';
require_once 'cancela_conta_receber.php';
require_once 'edita_conta_receber.php';
require_once 'estorna_conta_receber.php';
require_once 'filtro_contas_receber.php';
require_once 'filtro_del_contas_receber.php';
require_once 'filtro_mov_contas_receber.php';
require_once 'grid_contas_receber.php';
require_once 'grid_del_contas_receber.php';
require_once 'grid_mov_contas_receber.php';
require_once 'info_mov_conta_receber.php';

class TCadContasReceber extends TNotebookPage
{
...
{
    parent::__construct ('Contas a Receber', 'contas_receber.png');
    ...
    $vbox->pack_start ($this->filtro = new TFiltroContasReceber (array ($this, 'pega_dados')), false);
    $vbox->pack_start ($this->grid = new TGridContasReceber ($this));
    
    // contas baixadas
    $this->notebook->append_page ($vbox = new GtkVBox, new GtkLabel (' Baixas '));
    $vbox->pack_start ($this->mov_filtro = new TFiltroMovContasReceber (array ($this, 'pega_dados_mov')), false);
    $vbox->pack_start ($this->mov_grid = new TGridMovContasReceber ($this));

    // contas canceladas
    $this->notebook->append_page ($vbox = new GtkVBox, new GtkLabel (' Canceladas '));
    $vbox->pack_start ($this->del_filtro = new TFiltroDelContasReceber (array ($this, 'pega_dados_del')), false);
    $vbox->pack_start ($this->del_grid = new TGridDelContasReceber ($this));
    
    $this->notebook->connect ('switch-page', array ($this, 'notebook_switch_page'));
    $this->notebook->set_current_page (0);
    
    $this->lancar->set_sensitive (CheckPermissao ($this, 'lancar_conta_receber'));
    $this->info_mov->set_sensitive (CheckPermissao ($this, 'info_mov_conta_receber'));
    $this->imprimir->set_sensitive (CheckPermissao ($this, 'imprimir_contas_receber'));
}
...
function notebook_switch_page ($notebook, $pointer, $page_num)
{
    ...
    case 0: /* ABERTO */
    {
	$this->baixar->set_sensitive (CheckPermissao ($this, 'baixar_contas_receber'));
	$this->cancelar->set_sensitive (CheckPermissao ($this, 'cancelar_conta_receber'));
	...
	$this->estornar->set_sensitive (CheckPermissao ($this, 'estornar_conta_receber'));
	break;
    }
    case 2: /* CANCELADO */
    {
...
{
    $conta_receber = new TEditaContaReceber ($this);
    $conta_receber->show ();
}
...
function baixar_clicked ()
{
    ...
    $contas_receber = new TBaixaContasReceber ($this, $this->InfoLinha);
    if ($contas_receber->pega_dados ()) $contas_receber->show ();
}
...
function info_mov_clicked ()
{
    ...
    {
	$info_mov_conta_receber = new TInfoMovContaReceber ($this, $cur_grid->Valores [1]);
	if ($info_mov_conta_receber->pega_dados ()) $info_mov_conta_receber->show ();
    }
}
...
function cancelar_clicked ()
{
    ...
    {
	$conta_receber = new TCancelaContaReceber ($this, $this->grid->Valores [1]);
	$conta_receber->show ();
    }
}
...
function estornar_clicked ()
{
    ...
    {
	$conta_receber = new TEstornaContaReceber ($this, $this->mov_grid->Valores [0]);
	$conta_receber->show ();
    }
}
...
function imprimir_clicked ()
{
	...
	$filtro = $this->filtro;
	$grid = $this->grid;
	$extra = ' Tb_Contas_Receber.Cod_S_Sit = 1 ';
	break;
    case 1:
	$modulo = 'mov_contas_receber';
	$filtro = $this->mov_filtro;
	$grid = $this->mov_grid;
	$extra = ' Tb_Mov_Contas_Receber.Inativo = 0 ';
	break;
    case 2:
	$modulo = 'del_contas_receber';
	$filtro = $this->del_filtro;
	$grid = $this->del_grid;
	$extra = ' Tb_Del_Contas_Receber.Inativo = 0 ';
	break;
    };
...
function pega_dados_mov ()
{
    ...
    $sql = 'SELECT * FROM Vw_Mov_Contas_Receber';
...
function pega_dados_del ()
{
    ...
    $sql = 'SELECT * FROM Vw_Del_Contas_Receber';
