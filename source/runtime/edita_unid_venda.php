...
class TEditaUnidVenda extends TJanela
{
...
{
    parent::__construct ($operacao == 'i' ? 'Unidade de Venda - Incluir' : 'Unidade de Venda - Alterar',
                         null, null, 'unid_vendas.png');
    ...
    $hbox->pack_start ($this->nome = new AEntry (true, true, 'Tb_Unid_Vendas', 'Nome'));
    
    // descricao
    $this->pack_start ($hbox = new GtkHBox, false);
    $hbox->pack_start (new GtkLabel (latin1 (' Descrição: ')), false);
    $hbox->pack_start ($this->descricao = new AEntry (true, true, 'Tb_Unid_Vendas', 'Descricao'));
...
function grava_dados ()
{
    ...
    if ($this->operacao == 'i') $sql = 'call SP_Unid_Venda_Inc';
    else $sql = 'call SP_Unid_Venda_Alt';
