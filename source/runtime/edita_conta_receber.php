...
class TEditaContaReceber extends TJanela
{
...
{
    parent::__construct ($operacao == 'i' ? 'Contas a Receber - Incluir' : 'Contas a Receber - Alterar',
                         null, null, 'contas_receber.png');
    ...
    $hbox->pack_start ($this->clientes = new TClientes ($this));
    
    // num doc
    $this->pack_start ($hbox = new GtkHBox, false);
    $hbox->pack_start ($this->num_doc = new AEntry (true));
    $this->num_doc->label->set_text (' Num. do Doc.: ');
    ...
    $this->clientes->entry->grab_focus ();
}
...
function grava_dados ()
{
    ...
    if ($this->operacao == 'i') $sql = 'call SP_Conta_Receber_Inc';
    else $sql = 'call SP_Conta_Receber_Alt';
    
    $data = $sql . '(' .
	    String ($this->tipo_doc->CodTipoDoc) . ',' .
	    String ($this->filial->CodFilial) . ',' .
	    String ($this->clientes->CodCliente) . ',' .
	    String ($num_doc) . ',' .
	    String ($parcela) . ',' .
	    String ($vencimento) . ',' .
	    String ($valor) . ',' .
