...
require_once 'grid_info_mov_conta_receber.php';

class TInfoMovContaReceber extends TJanela
{
...
function pega_dados ()
{
    ...
    // Lançamentos
    if (!$db->multi_query ('SELECT * FROM Vw_Contas_Receber WHERE Id = ' . $this->CodConta)) return;
    ...
    // Baixas
    if (!$db->multi_query ('SELECT * FROM Vw_Mov_Contas_Receber WHERE CodConta = ' . $this->CodConta)) return;
    ...
    // Cancelamentos
    if (!$db->multi_query ('SELECT * FROM Vw_Del_Contas_Receber WHERE CodConta = ' . $this->CodConta)) return;
