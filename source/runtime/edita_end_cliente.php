...
require_once 'classes/tipo_endereco.php';

class TEditaEndCliente extends TJanela
{
 ...
 *  a => alterar
 */
function __construct ($Parent, $operacao = 'i', $CodCliente, $CodId = null)
{
...
function grava_dados ()
{
    ...
    if ($this->operacao == 'i') $sql = 'call SP_End_Cliente_Inc';
    else $sql = 'call SP_End_Cliente_Alt';
    
    $data = $sql . '(' .
	    String ($this->CodId) . ',' .
	    String ($this->CodCliente) . ',' .
...
function pega_dados ()
{
    ...
    if (!$db->multi_query ('SELECT * FROM Vw_End_Clientes WHERE Id = ' . $this->CodId)) return;
