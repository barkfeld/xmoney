...
class TEditaTransportadora extends TJanela
{
 ...
 *  a => Alterar
 */
function __construct ($Parent, $operacao = 'i', $CodTrans = null)
{
    parent::__construct ($operacao == 'i' ? 'Transportadora - Incluir' : 'Transportadora - Alterar',
			 null, null, 'transportadoras.png');
    
    $this->Parent = $Parent;
    $this->operacao = $operacao;
    $this->CodTrans = $CodTrans;
    
    $GLOBALS ['XMONEY_FIELD'] = 'Cod_S_Trans';
    $GLOBALS ['XMONEY_FIELD_ID'] = $CodTrans ? $CodTrans : -1;
    ...
    $hbox->pack_start ($this->nome = new AEntry (true, true, 'Tb_Transportadoras', 'Nome'));
    
    // CPF / CNPJ
    $vbox->pack_start ($hbox = new GtkHBox);
    $hbox->pack_start ($this->lbl_cpf = new GtkLabel (' CPF:  '), false);
    $hbox->pack_start ($this->cpf = new AEntry (true, true, 'Tb_Transportadoras', 'CPF'));
    ...
    $hbox->pack_start ($this->limite_entrega = new TFloat, false);
    $this->limite_entrega->label->set_text (' Limite de Entrega: ');
    ...
    // extra
    $this->limite_entrega->set_next_focus ($this->ok);
...
function grava_dados ()
{
    ...
    $limite_entrega = $this->limite_entrega->get_text ();
    $ativo = $this->ativo->get_active ();
    
    if ($this->operacao == 'i') $sql = 'call SP_Transportadora_Inc';
    else $sql = 'call SP_Transportadora_Alt';
    ...
    $data = $sql . '(' .
	    ...
	    String ($this->pessoa->CodTipoPessoa) . ',' .
            ...
            String ($url) . ',' .
	    String ($anotacoes) . ',' .
	    String (CommaToPoint ($limite_entrega)) . ',' .
	    String ($ativo) . ',' .
	    $GLOBALS ['CodUsuario'] .
	    ');';
...
function pega_dados ()
{
    ...
    if (!$db->multi_query ('SELECT * FROM Vw_Transportadoras WHERE Id = ' . $this->CodTrans)) return;
    ...
    $this->limite_entrega->set_text (PointToComma ($line ['LimiteEntrega']));
...
function limpa_campos ()
{
    ...
    $this->limite_entrega->set_text ('');
    
    $this->nome->set_focus ();
}

}; // TEditaTransportadora
