...
require_once 'edita_end_fornecedor.php';
require_once 'grid_end_fornecedores.php';

class TEndFornecedores extends TNotebookPage
{
...
{
    $endereco = new TEditaEndFornecedor ($this, 'i', $this->CodFor);
    $endereco->show ();
}
...
function alterar_clicked ()
{
    ...
    {
	$endereco = new TEditaEndFornecedor ($this, 'a', $this->CodFor, $this->grid->Valores [0]);
...
{
    if ($this->grid->pega_dados ()) impressao_geral ('end_fornecedores');
}
...
function pega_dados ()
{
    ...
    if (!$db->multi_query ('SELECT * FROM Vw_End_Fornecedores WHERE CodFor = ' . $this->CodFor)) return;
