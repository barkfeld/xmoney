...
require_once 'edita_sit_produto.php';
require_once 'grid_sit_produtos.php';

class TCadSitProdutos extends TNotebookPage
{

function __construct ()
{
    parent::__construct (latin1 ('Situações de Produto'), 'sit_produtos.png');
    ...
    // grid
    $this->pack_start ($this->grid = new TGridSitProdutos ($this));
    
    $this->incluir->set_sensitive (CheckPermissao ($this, 'incluir_sit_produto'));
    $this->alterar->set_sensitive (CheckPermissao ($this, 'alterar_sit_produto'));
    $this->excluir->set_sensitive (CheckPermissao ($this, 'excluir_sit_produto'));
    $this->imprimir->set_sensitive (CheckPermissao ($this, 'imprimir_sit_produtos'));
...
{
    $edita_sit_produto = new TEditaSitProduto ($this);
    $edita_sit_produto->show ();
}
...
function editar_clicked ()
{
    ...
    {
	$edita_sit_produto = new TEditaSitProduto ($this, 'a', $this->grid->Valores [0]);
	if ($edita_sit_produto->pega_dados ()) $edita_sit_produto->show ();
    }
}
...
{
    if ($this->grid->pega_dados ()) impressao_geral ('sit_produtos');
}
