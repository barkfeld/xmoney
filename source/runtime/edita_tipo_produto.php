...
class TEditaTipoProduto extends TJanela
{
...
{
    parent::__construct ($operacao == 'i' ? 'Tipo de Produto - Incluir' : 'Tipo de Produto - Alterar',
                         null, null, 'tipos_produto.png');
    ...
    $hbox->pack_start ($this->nome = new AEntry (true, true, 'Tb_Tipos_Produto', 'Nome'));
    
    // abreviacao
    $this->pack_start ($hbox = new GtkHBox, false);
    $hbox->pack_start (new GtkLabel (latin1 (' Abreviação: ')), false);
    $hbox->pack_start ($this->abreviacao = new AEntry (true, true, 'Tb_Tipos_Produto', 'Abreviacao'));
...
function grava_dados ()
{
    ...
    if ($this->operacao == 'i') $sql = 'call SP_Tipo_Produto_Inc';
    else $sql = 'call SP_Tipo_Produto_Alt';
